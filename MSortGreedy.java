...
public class MSortGreedy{
    public static void main(String[] args) {
        int[] make_spans = {};
        for (int i=0;i <= args.length;i++){
            make_spans[i] = get_makespan(args[i]);
        }
    }

    public static int get_makespan(String filename) {
        ...
        Task [] table;//new table to store the tasks
        Sort s = new Sort();
        try {
                ...
                if (line.length() != 0){//if the line contains numbers
                    ...
                    while (nt<t && line != null) {//while there are more tasks you have to read and there is a line
                        if (line.length() != 0){
                            ...
                            table[nt]=y;//add the task in the table
                    ...
                    if (flag) {//if everything is correct with the txt
                        s.quickSort(table,0,table.length-1);//sort the tasks in the table using quickSort
                        for (i =0; i<table.length;i++) {
                            x = pq.getmax();//get and remove the processor with the highest priority
                            x.addTask(table[i]);//add to this processor the task
